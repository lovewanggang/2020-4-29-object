...
<script type="text/javascript">
	...
	var id = getQueryString("id");
		$.ajax({
			...
			success:function(data){
				$("#appid").val(data.account.wxAppID);
				$("#appsecret").val(data.account.wxAppSecret);
				...
				$("#companyName").val(data.account.companyName);
				$("#owner").val(data.account.owner);
				$("#phone").val(data.account.phoneNum);
				$("input[name=sex][value='"+data.account.taocan+"']").attr("checked",true);
				$("#bank").val(data.account.bank);
				$("#bankAccount").val(data.account.bankAccount);
				$("#scope_of_business").val(data.account.scope_of_business);
				if(data.account.business_license != null){
				$("#BLpic").attr("src",data.account.business_license);
				}else{
				$("#BLpic").attr("src","img/pc_img/tp.png");
				}
				$("#datemax").val(data.account.registerDate);
			},
			error:function (data){
				alert("获取数据失败");
			}
		...
		<form class="form form-horizontal">
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input id="appid" type="text" class="input-text" value="" placeholder="" disabled="disabled">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3">APPSECRET：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input id="appsecret" type="text" class="input-text" value="" placeholder="" disabled="disabled">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3">账户ID：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input id="accountId" type="text" class="input-text" value="" placeholder="" disabled="disabled">
				</div>
			</div>
			<br/>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input disabled="disabled" id="accountName" type="text" class="input-text" value="" placeholder="">
				</div>
			</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input disabled="disabled" id="password" type="text" class="input-text" value="" placeholder="">
				</div>
			</div>
